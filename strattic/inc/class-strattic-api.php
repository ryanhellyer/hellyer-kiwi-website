...
class Strattic_API extends Strattic_Core {
	...
	public function get_wp_rest_api_response( $request = '' ) {
		...
		// Mapping array to the format used by the default WordPress API
		$urls = array();
		foreach ( $paths as $key => $path ) {
			$urls[][ 'link' ] = home_url() . $path;
		}
...
*/
		...
		$urls = array_merge( $urls, $this->get_extra_pagination() );
		...
		$urls = array_merge( $urls, $this->comeet_redirects() );
		/*
		*/
//print_r( $urls );die;
...
//				$paths = array_merge( $paths, $new_paths );
	...
	public function get_extra_pagination() {
		...
		$start_urls = array(
			'Home URL'       => home_url(),
			'Blog posts URL' => get_permalink( get_option( 'page_for_posts' ) ),
		);

		foreach ( $start_urls as $explanation => $url ) {

			$pagination_urls = $this->get_archive_pagination_urls( $url );
			$urls = array_merge( $urls, $pagination_urls );
		...
		foreach ( $this->taxonomy_term_ids as $taxonomy => $terms ) {
			...
			foreach ( $terms as $term_id ) {
				...
				if ( get_option( 'permalink_structure') ) {
					...
					$paths[] = get_term_link( $term_slug, $taxonomy );
					/*
	...
	public function comeet_redirects() {
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				// Get things from output (not sure what these are, hence calling them "things")
				$characters = json_decode( $data ); // decode the JSON feed
				foreach ( $characters as $character ) {

					if ( isset( $options['advanced_search'] ) && '1' === $options['advanced_search'] ) {
						$things[] = $character->department;
					} else {
						$things[] = $character->location;
					}

				}
				$ar = array_replace( $things, array_fill_keys( array_keys( $things, null ), '' ) );
				$things = array_count_values( $ar );

				// If things exist, then create URLs from them
				if ( $things ) {

					$i = 0;
					foreach ( $things as $item => $count ) {
						$i ++;

						$url = home_url() . '/co/' . strtolower( clean( $item ) ) . '/all';
						$urls[] = $url;
