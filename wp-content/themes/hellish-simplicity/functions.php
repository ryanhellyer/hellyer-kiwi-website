...
class Hellish_Setup {
	...
	public function register_setting() {
		 ...
		 * Grabs site name and adds <span> tags to second word to provide two tone header colour)
		 */
		$site_name = get_bloginfo( 'name' );
		$site_name = explode( ' ', $site_name );
		$default_header_text = '';
		$count = 0;
		foreach( $site_name as $word ) { // Loop through each word
			$count++;
			if ( 2 == $count ) {
				$word = '<span>' . $word . '</span>';
				$count = 0;
			}
			$default_header_text .= $word;
		}
		add_option( 'header-text', $default_header_text );
