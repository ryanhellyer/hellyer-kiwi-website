...
if ( true === $this->the_slug_exists( $post_name, 'invoice' ) ) {
	echo 'same slug';die;
}
...
add_action( 'save_post', array( $this, 'meta_invoice_from_boxes_save' ), 10, 2 );
//echo $post_id . ': ' . $invoice_number;echo "\n............\n";print_r( $result );die;
	...
	if ( isset( $_POST['_invoice_currency'] ) ) {
		...
		if ( isset( $_POST['_invoice_issuance_date'] ) ) {
			$invoice_issuance_date = date( self::DATE_FORMAT, absint( strtotime( $_POST['_invoice_issuance_date'] ) ) ) . ' 00:00:00';
			update_post_meta( $post_id, '_invoice_issuance_date', $invoice_issuance_date );
		}
