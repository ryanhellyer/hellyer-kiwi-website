...
if ( 'Applications' === get_the_title() ) {
	...
	echo '
	<input type="hidden" id="src-driver-name" value=" ' . esc_attr( wp_get_current_user()->display_name ) . '" />
	<p>
		<label>Team name (maximum of two drivers per team, maybe join an already existing team?)</label>
		<input type="text" id="src-team-name" />
	</p>
	<p>
		<label>Vehicle (<a href="' . esc_url( home_url() ) . '/available-cars/">click here for a list of available vehicles</a>)</label>
		<input type="text" id="src-vehicle" />
	</p>
	<p>
		<label>Tell us a little about you</label>
		<input type="text" id="src-description" />
	</p>
	<p>
		<label>Car number (<a href="' . esc_url( home_url() ) . '/championship/">check if your number is available</a>)</label>
		<input type="text" id="src-number" />
	</p>
