...
if ( isset( $_GET['all'] ) ) {
	$contacted = ''; // If all are requested, then don't care about who was contacted previously.
}
...
foreach ( $users as $key => $data ) {
	...
	// If all requested, then still don't want to include those who don't want notifications.
	if ( isset( $_GET['all'] ) ) {
		$driver_id = $driver->ID;
		if ( 'no' === get_user_meta( $driver_id, 'receive_notifications', true ) ) {
			unset( $listed_drivers[$driver_name] );
		}
	} else if ( isset( $listed_drivers[$driver_name] ) ) {
		unset( $listed_drivers[$driver_name] );
	}
