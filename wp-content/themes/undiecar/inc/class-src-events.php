...
*/
	...
	private function get_simplified_time( $time, $round = 2 ) {
		...
		if ( isset( $exploded[ $count ] ) ) {
			...
			$before_decimal = explode( '.', $exploded[ $count ] );
			if ( 1 === strlen( $before_decimal[0] ) ) {
				$exploded[ $count ] = '0' . $exploded[ $count ];
			}
		}
		...
		// If double zeros, the result is null and should be set as such.
		if ( '00' === $time ) {
			$time = '';
		}
