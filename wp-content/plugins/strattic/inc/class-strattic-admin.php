...
class Strattic_Admin extends Strattic_Core {
	...
	public function __construct() {
        ...
        add_action( 'admin_footer',          array( $this, 'completed_box' ), 99999 ); 
...
s								<th>
			...
			<table class="strattic-table wp-list-table widefat fixed striped pages">
			<?php
				...
				?>
				 id="distribution-<?php echo esc_attr( $distribution_id ); ?>">
					<th scope="row">
						...
						<div class="strattic-info-box">
							<p class="strattic-button-box">
								<a data-distribution-type="<?php echo esc_attr( $type ); ?>" data-distribution-id="<?php echo esc_attr( $distribution_id ); ?>" class="strattic-publish button button-primary" ><?php esc_html_e( 'Publish', 'strattic' ); ?></a>
							</p>
	 ...
	 * This needs to be in the footer of every page incase it is triggered when not in the Strattic admin page.
	 */
...
<?php
	...
	public function add_toolbar_link( $wp_admin_bar ) {
		...
		$wp_admin_bar->add_menu(
			array(
				'id'    => 'strattic',
				'title' => '<span class="ab-icon"></span> <span class="ab-message">' . esc_html( $this->strattic_strings['publish'] ) . '</span>',
