...
class Strattic_URLs extends Strattic_Core {
	...
	public function __construct() {
		...
		if ( strpos( $this->get_current_path(), '/strattic-api-request' ) !== false ) {
			add_action( 'template_redirect', array( $this, 'get_direct_api_request' ), 1 );
		}
	...
	public function get_direct_api_request( $distribution_id = null ) {
		...
		/*
		// Get stats
		$strattic_stats = new Strattic_Stats;
		$stats = $strattic_stats->get_stats();
		*/
			...
			while ( $query->have_posts() ) {
				...
				if (
					...
					if ( 'trash' === get_post_status() ) {
						$url = str_replace( '__trashed/', '/', get_the_permalink() );
					} else {
