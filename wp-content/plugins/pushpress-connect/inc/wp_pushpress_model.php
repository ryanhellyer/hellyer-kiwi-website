...
class Wp_Pushpress_Model {
	...
	public function get_client() {
		...
		if ( strlen( trim( $secret_code ) ) ) {
			try {
				if( ! $client = get_transient( 'pp_client_info_1' ) ) {
					...
					set_transient( 'pp_client_info', $client, PUSHPRESS_CACHE_1 );
				}
			}
			catch ( Exception $e ) {
				try {
	...
	public function get_products() {
		...
		$cache_key = 'pushpress_products_list';
		if ( ! $productsList = get_transient( $cache_key ) ) {
			...
			*/
			...
			set_transient( $cache_key, $productsList, PUSHPRESS_CACHE_1 );
		}
...
////CACHE HERE
		$cache_key = 'pushpress_products_by_id_' . $id;
		if ( ! $productsList = get_transient( $cache_key ) ) {
	...
	public function get_products_by_category( $category ) {
		...
		$cache_key = 'pushpress_products_by_category_' . $category;
		if ( ! $productsList = get_transient( $cache_key ) ) {
	...
	public function get_plans( $atts = array() ) {
		...
		$cache_key = 'pushpress_plans_' . md5( print_r( $atts, true ) );
		if ( ! $plansList = get_transient( $cache_key ) ) {
			...
			*/
			...
			set_transient( $cache_key, $plansList, PUSHPRESS_CACHE_1 );
		}
	...
	public function get_plans_by_id( $id ) {
		...
		$cache_key = 'pushpress_plans_by_id_' . $id;
		if ( ! $plansList = get_transient( $cache_key ) ) {
	...
	public function get_plans_for_help() {
		...
		$cache_key = 'pushpress_plans_for_help';
		if ( ! $plansList = get_transient( $cache_key ) ) {
	...
	public function get_categories_for_help() {
		...
		$cache_key = 'pushpress_categories_for_help';
		if ( ! $categories = get_transient( $cache_key ) ) {
			...
			*/
			...
			set_transient( $cache_key, $categories, PUSHPRESS_CACHE_1 );
		}
	...
	public function get_events( $isEvent = "event", $atts ) {
		...
		$cache_key = 'pushpress_events_' . md5( $isEvent . print_r( $atts, true ) );
		if ( ! $eventsList = get_transient( $cache_key ) ) {
			...
			*/
			...
			set_transient( $cache_key, $eventsList, PUSHPRESS_CACHE_1 );
		}
	...
	public function get_event_by_id( $id ) {
		...
		$cache_key = 'pushpress_event_by_id_' . $id;
		if ( ! $eventsList = get_transient( $cache_key ) ) {
	...
	public function get_events_for_help() {
		...
		$cache_key = 'pushpress_events_for_help';
		if ( ! $eventsList = get_transient( $cache_key ) ) {
	...
	public function get_schedules( $date, $timeNow, $atts ) {
		...
		$cache_key = 'pushpress_schedules_' . md5( $date . $timeNow . print_r( $atts, true ) );
		if ( ! $schedulesList = get_transient( $cache_key ) ) {
			...
			set_transient( $cache_key, $schedulesList, PUSHPRESS_CACHE_1 );
		}
	...
	public function get_workout( $date, $timeNow, $atts ) {
		...
		try {
			...
			if ( ! $results = get_transient( $cache_key ) ) {
				...
				set_transient( $cache_key, $results, PUSHPRESS_CACHE_1 );
	...
	public function get_leads() {
		...
		try {
			...
			if ( false === ( $integration_settings = get_transient( 'lead_settings' ) ) ) {
				$integration_settings = Pushpress_Client::settings( 'lead_capture' );
				set_transient( 'lead_settings', $integration_settings, 300 );
			}
			...
			*/
			...
			foreach ( $integration_settings->data as $item ) {
			...
			*/
			...
			foreach ( $referral_sources->data as $item ) {
				$referral[] = array(
					'id'              => $item['id'],
					'name'            => $item['name'],
					'show_staff_list' => $item['show_staff_list']
				);
			}
		} catch ( Exception $e ) {
			return;
	...
	public function pushpress_marketing_integrations() {
	...
	public function facebook_integrations() {
		...
		try {
			...
			$cache_key = 'pushpress_facebook_integrations';
			if ( ! $integration_settings = get_transient( $cache_key ) ) {

				$integration_settings = Pushpress_Client::settings( 'integration' );

				set_transient( $cache_key, $integration_settings, PUSHPRESS_CACHE_1 );
			}
	...
	public function facebook_metrics() {
		...
		try {
			...
			$cache_key = 'pushpress_facebook_metrics';
			if ( ! $settingsObj = get_transient( $cache_key ) ) {

				$settingsObj = Pushpress_Client::settings( 'metrics' );                

				set_transient( $cache_key, $settingsObj, PUSHPRESS_CACHE_1 );
			}
	...
	public function get_staff() {
		...
		try {
			...
			$cache_key = 'pushpress_staff';
			if ( ! $staff = get_transient( $cache_key ) ) {

				$staff = Pushpress_Customer::all(
					array(
						'is_staff' => 1,
						'active'   => 1,
					)
