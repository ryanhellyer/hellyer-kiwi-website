...
class WeDevs_Settings_API {
	...
	function callback_cat2cat( $args ) {
		...
		foreach ( $args['options'] as $key => $label ) {
			...
			$html .= sprintf( '<select id="%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" />', $args['section'], $args['id'], $key );

			$html .= '<option value="0"></option>';
			$html .= '<option value="new-mediahub-category|' . $key . '|' . $label . '"> - als nieuwe categorie toevoegen - </option>';
...
//echo $time;die;
		...
		wp_schedule_event( current_time ( 'timestamp' ), $time, 'mh_event_hook' );
