...
abstract class Strattic_Core {
	...
	protected function get_current_site_strattic_id() {
		...
		// Grab from cache if recently queried.
		$key = 'strattic-site-id';
		if ( false === ( $site_id = get_transient( $key ) ) ) {
			$site_data = $this->get_current_site_api_data();
			if ( isset( $site_data['id'] ) ) {
				$site_id = absint( $site_data['id'] );
			} else {
				$site_id = null;
			}

			set_transient( $key, $site_id, MINUTE_IN_SECONDS );
		}
	...
	/**
	 * Determine distribution type given ID
	 *
	 * @param  mixed $distribution_id
	 *
	 * @return void
	 */
	public function get_distribution_type( $distribution_id ) {
