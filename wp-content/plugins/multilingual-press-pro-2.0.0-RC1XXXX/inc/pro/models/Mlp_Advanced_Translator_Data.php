...
class Mlp_Advanced_Translator_Data
	...
	public function save( $post_id, WP_Post $post ) {
		...
		$post_meta = $this->basic_data->get_post_meta_to_transfer( $post_id );
		$post_meta = apply_filters( 'mlp_pre_save_post_meta', $post_meta );

		// Set source
		$this->basic_data->set_source_id( $post_id, $source_blog_id );
		$this->basic_data->set_linked_element( $post_id, $post_id, $source_blog_id, '', $source_blog_id );

		foreach ( $_POST[ $this->name_base ] as $remote_blog_id => $post_data ) {
			...
			$new_post   = $this->create_post_to_send( $post_data, $post_type );
			$sync_thumb = ! empty ( $post_data[ 'thumbnail' ] );
			$update     = ! empty ( $post_data[ 'remote_post_id' ] ) && 0 < $post_data[ 'remote_post_id' ];
			$new_id     = $this->sync_post( $new_post, $post_id, $source_blog_id, $remote_blog_id, $update );
	...
	private function sync_post(
		...
		$this->basic_data->set_linked_element( $new_id, $post_id, $source_blog_id, '', $remote_blog_id );
	 ...
	 * @return array
	 */
	private function create_post_to_send( Array $post_data, $post_type ) {
		...
		if ( ! empty ( $post_data[ 'remote_post_id' ] ) ) {
			...
			return apply_filters( 'mlp_pre_update_post', $new );
		}
	...
	private function get_remote_post_title( Array $data ) {
		...
		if ( isset ( $_POST[ 'post_title' ] ) )
			return (string) $_POST[ 'post_title' ];
	...
	private function get_remote_post_content( Array $data ) {
		...
		if ( isset ( $_POST[ 'post_content' ] ) )
			return (string) $_POST[ 'post_content' ];

		return '';
	}

	/**
	 * Get all blogs (sites) related (linked) to the current.
	 *
	 * @return array
	 */
	private function get_related_blogs() {

		$blogs = get_option( 'inpsyde_multilingual_blog_relationship' );

		// We don't need to save because there is no related blog
		if ( empty ( $blogs ) or ! is_array( $blogs ) )
			return array ();
	...
	private function is_valid_save_request( WP_Post $post ) {
		...
		if ( empty ( $_POST[ $this->name_base ] ) )
			return FALSE;

		if ( ! is_array( $_POST[ $this->name_base ] ) )
			return FALSE;
