...
class Strattic_API {
	...
	public function get_all_posts( $request ) {
		...
		foreach ( get_post_types( array(), 'names', 'and' ) as $post_type ) {
			$post_types[] = $post_type; // Re-keying so that we have a numeric counter
		}

		$count = $post_type_max = 0;
		$previous_post_type = '';
